function results = optSwapPool(model, opt)
...
% results
    ...
    if ~isfield(opt,'pastSolutions'), opt.pastSolutions = {};
    else 
        if opt.knockType==2 || opt.knockType==0
            %error('pastSolutions not compatible with optSwap or OptKnock');
        end
    end
    if ~isfield(opt,'maxW'),
    ...
    if (knockType==0 || knockType==1)
        ...
        elseif (knockType == 1)
            ...
            % block past solutions
            for i=1:length(opt.pastSolutions) %%%%% ONLY working for RobustKnock
                                              % ones for past solutions in the integer region
                someZeros = sparse(1, size(A3, 2) - ySize);
                pastSolutionRow = -ismember(yInd, find(ismember(consModel.rxns, opt.pastSolutions{i})))';
                % -sum(past solution integers) <= -1...not all knocked out
                A3 = [A3; someZeros, pastSolutionRow];
                B3 = [B3; -1];
            end
