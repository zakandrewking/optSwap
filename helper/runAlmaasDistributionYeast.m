function runAlmaasDistributionYeast
    ...
    aerString = {'aerobic', 'aerobic', 'aerobic', 'aerobic', ...
    ...
    for i=1:s
        fprintf('Case %d of %d\n', i, length(subs));
        model = setupModel('yeast6',subs{i},aerString{i});
        options.subs = subs{i};
    ...
    fileId = fopen('almaas_yeast6_all_dh_output_pFBA.tsv', 'w');
    fprintf(fileId, 'aerobic\t\t\t\t\t\t\t\tanaerobic\nglc\t\tD-xyl\t\tgylc\t\tL-arab\t\tglc\t\tD-xyl\t\tgylc\t\tL-arab\n');
