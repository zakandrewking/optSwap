% for submission
% TODO replace q and s with s and t?

function results = optSwap_submit(model, opt)
...
% results
    ...
    if ~isfield(opt,'maxW'), opt.maxW = 1e7; end
    if ~isfield(opt,'biomassRxn')
        opt.biomassRxn = model.rxns(model.xc~=0);
    end
