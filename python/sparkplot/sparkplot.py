...
class Sparkplot:
    ...
    def process_args(self):
        ...
        parser.add_option("-f", "--file_format", help="file format",
                         default="png", dest="file_format")
    ...
    def generate_output_file(self):
        ...
        The AGG backend will automatically append file_format to the file name
        """
        if not self.output_file:
            self.output_file = os.path.splitext(self.input_file)[0]
        file_with_extension = self.output_file + '.' + self.file_format
        if self.verbose:
            print "Generating output file " + file_with_extension
        savefig(file_with_extension, format=self.file_format)
